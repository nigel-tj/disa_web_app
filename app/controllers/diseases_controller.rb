class DiseasesController < ApplicationController
  ...
  def overall
    ...
    @end_of_month.times do |date|
      ...
      @number_of_diagnosed_male = Condition.where(" gender = ? AND created_at >= ? AND created_at <= ?", "male", @start_day, @end_day ).count
      @number_of_diagnosed_female = Condition.where(" gender = ? AND created_at >= ? AND created_at <= ?", "female", @start_day, @end_day ).count      
      @female_chart << [ "#{@start_day}"  , @number_of_diagnosed_female]
      @male_chart << [ "#{@start_day}"  , @number_of_diagnosed_male]
    end
    puts @male_chart
    puts @female_chart
